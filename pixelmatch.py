...
def pixelmatch(img1, img2, width: int, height: int, output=None, options=None):
    ...
    if len(img1) != len(img2) or (output and len(output) != len(img1)):
        raise Exception("Image sizes do not match.", len(img1), len(img2), len(output))

    if len(img1) != width * height * 4:
        raise Exception("Image data size does not match width/height.", len(img1), width * height * 4)
